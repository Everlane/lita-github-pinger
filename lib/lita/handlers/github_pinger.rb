      ...
      def act_on_pr_labeled(body, response)
        ...
        if config.enable_round_robin
          ...
          message_for_reviewer = <<~eos
            You’re next in line to look at a PR: #{body["pull_request"]["html_url"]}
            There’s no obligation to submit a review, but take a look and familiarize yourself with the code as time allows.
